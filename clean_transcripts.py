...
def clean_transcript_file(file_path):
    """
    Clean a single transcript file by removing utterance numbers, timestamps, and line breaks.
    Keep only the actual spoken text content.
    """
    try:
        ...
        for block in blocks:
            ...
            if len(lines) >= 3:  # Should have: number, timestamp, text (and possibly more text lines)
                ...
                text_lines = lines[2:]
                # Join all text lines and add to our cleaned text
                text_content = ' '.join(text_lines).strip()
                if text_content:  # Only add non-empty content
                    cleaned_text.append(text_content)
        
        # Join all text with spaces to create one continuous text
...
def main():
    ...
    print("This script removes utterance numbers, timestamps, and line breaks from transcript files.")
    print()
